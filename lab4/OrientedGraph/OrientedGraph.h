...
public:
    ...
    using NeighborSet = std::set<T>;
    using AdjacencyMap = std::map<T, NeighborSet>;

private:
    AdjacencyMap AdjList;

public:
    using VertexIterator = GraphImpl::VertexIterator<OrientedGraph<T>>;
    using ConstVertexIterator = VertexIterator;
    using ReverseVertexIterator = std::reverse_iterator<VertexIterator>;
    
    using EdgeIterator = GraphImpl::EdgeIterator<OrientedGraph<T>>;
    
    using AdjacentIterator = GraphImpl::AdjacentIterator<OrientedGraph<T>>;
    using ConstAdjacentIterator = AdjacentIterator;
    using ReverseAdjacentIterator = std::reverse_iterator<AdjacentIterator>;

    using IncidentIterator = GraphImpl::IncidentIterator<OrientedGraph<T>>;
    ...
    EdgeIterator BeginEdge() const { return EdgeIterator(AdjList.begin(), AdjList.end(), AdjList.begin()); }
    EdgeIterator EndEdge() const { return EdgeIterator(AdjList.end(), AdjList.end(), AdjList.begin()); }
